...
2. Вычисляются нормы пробной кривой по напряжению и току отдельно:
  ...
  $normV = max(VoltagesRef)$ если $max(VoltagesRef) > 0.6 $; иначе $normV = 0.6 $

  $normC = max(CurrentsRef)$ если $max(CurrentsRef) > 0.0002 $; иначе $normC = 0.0002 $
...
1. Для каждой точки $p$ пробной кривой находится расстояние по напряжению и току до сегмента, образованного точками $j$ и $j+1% тестовой кривой.

2. Среди всех полученных расстояний находится наименьшая сумма квадратов, то есть наиблежайший сегмент для точки $p$.

3. Среди всех наименьших расстояний от точек $p$ пробной кривой до тестовой кривой находятся максимальные расстояния по напряжению и току.